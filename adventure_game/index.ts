...
function findEnemy(round: number): {name: string, health: number, damage: {min: number, max: number}} {
    if (round == 1) {
        return enemies[0]
    }
...
let errorMessage : string = chalk.bgGrey("Pro Tip: Buy a good sword before playing");
...
while (keepPlaying) {
    ...
    let whatToDo : { what_to_do : string } = await inquirer.prompt({
        type: "list",
        name: "what_to_do",
