...
let loggedIn = false;
let exit = false;
let message = chalk.grey(`You first have to register your account to log in`);
while (!exit) {
    console.clear;
    console.log(chalk.bgGray(`International Bank`));
    console.log(message);
    const screenChose = await inquirer.prompt({
        type: "list",
        name: "response",
        message: "Main Menu",
        choices: [
