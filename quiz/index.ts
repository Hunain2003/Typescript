...
let playerDetails : {name: string} = await inquirer.prompt({
    type: "input",
    name: "name",
...
let quizQuestions : {[index: string] : string} = await inquirer.prompt([
    {
        type: "list",
        name: "q1",
...
let gotWrong : number[] = [0, 0, 0, 0, 0, 0]
let score : number = 0;
if (quizQuestions.q1 == "Jimmy") { score++; } else { gotWrong[0]++; }
if (quizQuestions.q2 == "3rd") { score++; } else { gotWrong[1]++; }
if (quizQuestions.q3 == "12") { score++; } else { gotWrong[2]++ }
if (quizQuestions.q4 == "50") { score++; } else { gotWrong[3]++ }
if (quizQuestions.q5 == "Yes") { score++; } else { gotWrong[4]++ }
if (quizQuestions.q6 == "45") { score++; } else { gotWrong[5]++ }
...
if (score < 6) {
   console.log(chalk.bgGray(`Here are the questions you got wrong`))
    for (let i: number = 0; i < gotWrong.length; i++) {
        if (gotWrong[i] == 1) {
