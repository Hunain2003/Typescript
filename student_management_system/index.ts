...
let keepSMSRunning : boolean = true;

console.log(chalk.bgGray("School Management System"))

let initialId : number = 10001;
    ...
    switch (answer.task) {
        case "Show all current Students":
            ...
            if (students.length > 0) {
                ...
                let selectStudent : { student: string } = await inquirer.prompt({
                    type: "list",
                    name: "student",
                ...
                let studentToGetDetailsOf : Student = students.filter((student, i) => {
        ...
        case "Create new Student":
            console.clear();
            const newStudent: { name: string } = await inquirer.prompt(
                [
                {
                    type: "input",
                    name: "name",
        ...
        case "Delete a Student":
            ...
            if (students.length > 0) {
                ...
                const studentToDelete : { response: string } = await inquirer.prompt({
                    type: "list",
                    name: "response",
                ...
                for (let i : number = 0; i < students.length; i++) { console.log(`${students[i].id}: ${students[i].name}`) }
            } else {
                                ...
                                if (Number.isNaN(fee.paid)) {
                                    ...
                                    let newStudentsList = students.map((student, i) => {
                                        if (student.id == Number(studentToUpdate.student.slice(0, 5))) {
                                            ...
                                            if (student.amountPayable == 0) { student.tutionFeePaid = true }
                                            console.clear()
